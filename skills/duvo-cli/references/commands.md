...
| `duvo connections update <id> [--name <n>] [--server-url <url>] [--auth-method <method>] [--header KEY=VALUE …] [--secret-header KEY=VALUE …] [--clear-headers] [--composio-mcp-id <id>\|--clear-composio-mcp-id] [--share\|--unshare] [--json]` | Update a connection. `--share` is manager+ only. Sending an empty `--secret-header` value keeps the existing secret.                                                  |
...
| `duvo oauth composio start --auth-config <id> --callback-url <url> [--auth-scheme <scheme>] [--auth-field key=value …] [--json]`                                          | Start a Composio-backed OAuth connection. Returns a redirect URL and `connected_account_id`. `--auth-field` is repeatable and requires `--auth-scheme` for non-OAuth schemes (e.g. `API_KEY`, `BEARER_TOKEN`). |
