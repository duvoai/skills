...
| Command                                                                                                                                                                                                                                                                                  | Purpose                                                                                                                                                                                                                                         |
| ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `duvo runs list [--limit <n>] [--offset <n>] [--sort-order <asc\|desc>] [--status <s>] [--agent <id>] [--automation <id>] [--user <id>] [--case-queue <id>] [--source <source>] [--search <query>] [--has-issues <true\|false>] [--since <iso>] [--until <iso>] [--count-only] [--json]` | List runs for your team with filters. `--user` requires Superadmin or above; other callers always see their own runs regardless of the value. `--since`/`--until` bound by completion time. `--count-only` returns just the matching Run total. |
...
| `duvo team use <team-id>`                                                                                             | Set the default team for the active profile. Subsequent commands resolve to this team unless overridden by `--team` or `DUVO_TEAM_ID`. Works for OAuth profiles and user-scoped API-key profiles; on a team-scoped API-key profile it's harmless but useless, since the server always resolves that key to its one pinned team regardless. |
| `duvo teams list [--json]`                                                                                            | List every team your credentials can act on. A team-scoped API-key caller sees exactly that one team; a user-scoped API-key caller (like an OAuth caller) sees every team its owner belongs to. Useful before `duvo team use`.                                                                                                             |
