...
description: >
  ...
  wants to fix a recurring failure on an Assignment. Reads the Run's
...
A **Run** is one execution of an Assignment. When a Run fails or produces the wrong outcome, the user wants two things:
...
- **Paste mode** — no Duvo MCP tools are available (e.g. Duvo's in-product chat surface). Ask the user to paste the Run ID, the SOP that was in effect, the final error or relevant transcript excerpt, and any Connection / case context. Work from what they share.
...
- The Assignment was carrying two Runs of work in one SOP (decomposition signal).
...
- `getRevision` — the **Build that was active for this Run** (use the `build_id` from `getRun`, not the Assignment's current Build). This contains the SOP the Assignment was actually running against.
...
- `listRuns` — to spot a pattern across recent Runs of the same Assignment.
- `getAgent` — Assignment-level config (name, current Build) when the user gave you an Assignment ID only.

**Always fetch the revision the Run ran against, not the Assignment's current revision.** The user may have edited the SOP since the failure; otherwise you'd diagnose a version of the SOP that wasn't running.
...
1. **Always:** the Run ID (so the user can cross-reference), the SOP that was in effect at the time of failure (not necessarily the current one — warn the user that an SOP edit since the failure may explain why their _current_ SOP looks fine), and the final error or last few transcript turns.
...
4. **If a recurring pattern:** how many recent Runs of the Assignment failed similarly, on which case shapes.
...
1. **Anchor on the Run.** Get the Run's `build_id`, status, and any top-level error, plus the SOP that was in effect. _API mode:_ call `getRun`, then `getRevision` with that `build_id`. _Paste mode:_ ask the user for the Run ID and the SOP that was in effect at the time.
...
3. **Missing terminal closure.** A queue-driven Run ended without `complete_case` / `fail_case` / `postpone_case` / `request_handover` on a branch. The case is now stuck. Evidence: the Run ended after a non-terminal action and the case wasn't released. Fix: add the terminal action to that branch of the SOP.
...
7. **Wrong decomposition.** One Assignment doing the work of two — the Run spans Connection domains and time horizons, gets confused, fails partway through. Evidence: SOP exceeds ~10 top-level steps with clear phase boundaries (real-time scan → wait → reminder cycle). Fix: split into two Assignments connected by `request_handover` or a queue.
...
This split is intentional. `job-debugger` finds the bug. `sop-writer` writes the fix. Mixing the two produces shallow rewrites and unanchored diagnoses.
...
- **Diagnosing against the current SOP** when the failed Run ran against an earlier Build. Always work from the revision that was actually in effect — pull it via `getRevision` in API mode, or warn the user in paste mode that an SOP edit since the failure may explain why the current SOP looks fine.
...
If the user asked for a pattern hunt across multiple Runs ("why does this Assignment keep failing"), return one diagnosis per recurring failure mode, ordered by frequency.

## Reading the request

1. **Find the Run (or Assignment, or Case) reference** in the conversation. If absent, ask before reading.
2. **Determine intent.** Single-Run depth ("why did _this_ Run fail") vs. pattern sweep ("why does _this Assignment_ keep failing"). The first wants depth on one Run; the second wants a sweep across recent Runs of the Assignment.
3. **Determine queue vs. standalone shape.** If the Run has a `case_id` (API mode: from `getRun`; paste mode: ask the user or look at the SOP for case-lifecycle tool calls), it's queue-driven — check terminal closure. Otherwise focus on the final tool call and its result.
...
- [ ] You worked from **the SOP that was in effect at the time of the Run**, not the Assignment's current SOP.
...
- [ ] Duvo terminology used: Assignment, Run, SOP, Connection, Files, Login, Setup.
